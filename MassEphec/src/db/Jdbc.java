...
public class Jdbc {
    ...
    public static void createTable(){
        try{
            ...
            create.executeUpdate();
            create = con.prepareStatement("CREATE TABLE IF NOT EXISTS "+TABLE2+"(`nom` VARCHAR(255) NOT NULL, `path` VARCHAR(255), `vie` INT, `x` INT, `y` INT, `armeLevel` INT, `armeExperience` INT, `mapLevel` INT, `classe` VARCHAR(255), PRIMARY KEY (`nom`) )ENGINE=InnoDB"); 
            create.executeUpdate();
        }catch(Exception e){System.out.println(e);}
        finally{
